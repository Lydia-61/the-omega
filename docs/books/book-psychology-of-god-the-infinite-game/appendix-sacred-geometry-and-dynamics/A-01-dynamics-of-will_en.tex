...
\subsection{A.1.1 Modified Action Principle}
...
\textbf{Definition of Will Term $\mathcal{F}$:}
...
\subsection{A.1.2 Intentional Schrödinger Equation (ISE)}
...
\begin{enumerate}
\item \textbf{$\hat{H}_0$}: Standard physical Hamiltonian (Hermitian operator), ensuring energy conservation and unitary evolution.

\item \textbf{$\hat{W}(t)$}: \textbf{Will Operator} (non-Hermitian term).

\[\hat{W}(t) = - \nabla_{\Psi} \mathcal{F}\]

It represents the ``pruning'' or ``guidance'' of will on the wave function.

\item \textbf{$i\kappa$}: Dissipation/selection coefficient. Due to the introduction of the imaginary term $i$, this term actually describes a process of \textbf{continuous measurement and selection} (Zeno Effect).
\end{enumerate}

\textbf{Dynamical Interpretation:}

\begin{itemize}
\item \textbf{When $\kappa = 0$}: The system undergoes standard unitary evolution (dreaming/subconscious state).

\item \textbf{When $\kappa > 0$}: The $\hat{W}$ term causes components in the wave function that are \textbf{far from ``true self''} (i.e., high free energy states) to decay exponentially (suppressed), while components consistent with ``true self'' are amplified (selected).
\end{itemize}
...
\subsection{A.1.3 Bayesian Correction of Collapse Probability}
...
\subsection{A.1.4 Conclusion: Will as a Force}
