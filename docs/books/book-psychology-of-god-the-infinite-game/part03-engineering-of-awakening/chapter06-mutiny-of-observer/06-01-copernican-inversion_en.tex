...
\subsubsection*{The First Revolution and the Second Inversion}
...
\begin{itemize}
\item \textbf{Classical picture}: The stage (spacetime) is already set, the script (laws) is already written, actors (us) just come on stage to perform.

\item \textbf{Quantum picture}: The stage is a fuzzy probability cloud; only when actors cast their gaze does the spotlight (reality) light up there.
\end{itemize}

\subsubsection*{Wheeler's ``Participatory Universe'' and QCA's Rendering Mechanism}
...
\textbf{Theorem 6.1 (Observer-Dependent Rendering):}
...
\begin{enumerate}
\item \textbf{No objective ``there''}: If you don't look at the moon, the moon indeed does not exist at a definite spatial position; it is just a probability wave packet.

\item \textbf{Measurement is creation}: When you observe, you are not discovering an established fact; you are \textbf{forcing} the universe to choose one possibility from countless possibilities to present to you.
\end{enumerate}

\subsubsection*{From NPC to Player: The Awakening of Power}
