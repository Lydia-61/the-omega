\section{Appendix B: QCA Simulation Guide}

\textbf{Appendix B: Guide to Simulating QCA Universes}

Physics is not just for deriving formulas; it is also for \textbf{running}. The "unitary computational universe" constructed in this book exists not only in abstract Hilbert space, but can also be fully simulated on classical digital computers (though with efficiency limitations).
...
\subsection{B.1 Basic Architecture of the Simulator}

A QCA simulator mainly consists of three core modules:

\begin{enumerate}
\item \textbf{State Register}: Stores the current global wave function $|\Psi(t)\rangle$.
\end{itemize}

\begin{enumerate}
\item \textbf{Evolution Engine}: Executes local unitary operators $\hat{U}$.

\begin{enumerate}
\item \textbf{Measurement Module}: Computes observables (such as position probability distribution, momentum spectrum).
\end{itemize}

\subsubsection{B.1.1 Data Structure}
...
\subsection{B.2 Evolution Algorithm: Split Operator Method}
...
\subsubsection{B.2.1 Code Implementation}
...
\subsection{B.3 Initial Conditions and Observation}
...
\subsection{B.4 Experimental Script: Verifying Zitterbewegung}

Now, let's run this universe and verify the core prediction of Chapter 5: \textbf{Massive particles tremble microscopically.}
...
Running the above code, you will see that \texttt{<x>} is not a straight line (stationary), but exhibits \textbf{high-frequency, small-amplitude sinusoidal oscillation} near the initial position.

\begin{itemize}
\item Oscillation frequency $\omega \approx 2\theta$.
\end{enumerate}

\begin{itemize}
\item This is the precise discrete reproduction of the Zitterbewegung phenomenon in the Dirac equation.

\begin{itemize}
\item If `theta` is set to 0 (photon), oscillation disappears, and the wave packet will split and fly toward both ends at speed $c$ (or remain stationary if not mixed).
\end{enumerate}

\subsection{B.5 Advanced Challenge: Curved Spacetime Simulation}

To simulate gravity in QCA (Chapter 4 content), we need to introduce \textbf{non-uniform refractive index}.

This can be achieved by making the mass parameter $\theta$ or translation operator $\hat{S}$ depend on position $x$.

\textbf{Modification Idea}:

In the \texttt{step} function, instead of using a uniform \texttt{self.theta}, use an array \texttt{self.theta_field[x]}.

\begin{itemize}
\item Near a "black hole," set `theta[x]` very large (close to $\pi/2$), which will cause $v_{int} \to c$, $v_{ext} \to 0$.
\end{enumerate}

\begin{itemize}
\item Running the simulation, you will find that wave packets entering this region will drastically slow down, wavelengths compress, exhibiting characteristics of gravitational redshift and spacetime curvature.

In this way, you are not just learning physics; you are \textbf{coding physics}. Every logic gate is a natural law of this toy universe.
