\section{4.2 Local Information Volume Conservation: Why Must Space Expand When Information Compresses Time?}
...
This section will solve the "half-angle problem" and derive the correct optical metric form by introducing the key principle of \textbf{"local information volume conservation."} This principle is not just a mathematical patch, but an inevitable corollary of QCA unitarity at the statistical mechanics level.

\subsection{4.2.1 Discrete Correspondence of Liouville Theorem}
...
In QCA, the counterpart is \textbf{unitary evolution preserves Hilbert space dimension}.
...
\textbf{Axiom (Local Information Volume Conservation)}:
...
\subsection{4.2.2 Antagonism Between Time Dilation and Space Expansion}

Consider a region with high local information processing density $\rho_{\text{info}}$ (e.g., containing massive objects).

According to Light Path Conservation, accelerated internal computation means external clock slows. We describe this time dilation using \textbf{refractive index $n(x) > 1$}:
...
If only time dilation occurred, total degrees of freedom in this spacetime region would decrease (because time dimension capacity shrinks). To satisfy volume conservation (Liouville constraint), spatial dimensions must undergo compensatory \textbf{expansion}.
...
\textbf{Conclusion}: If gravity causes time to slow by factor $n$, it must simultaneously cause space to expand by factor $n$.

\[d\tau = \frac{1}{n} dt, \quad dl = n dx\]

\subsection{4.2.3 Derivation of Optical Metric}
...
This is the famous \textbf{Optical Metric}, also called Gordon metric.
...
This \textbf{completely matches} the weak field expansion of Schwarzschild metric in isotropic coordinates in general relativity.

\subsection{4.2.4 Solving the Half-Angle Problem}
...
This section proves: \textbf{Spatial curvature of spacetime is the inevitable companion of time dilation.}

As long as we acknowledge:

\begin{enumerate}
\item Gravity originates from local differences in information processing rates (Light Path Conservation);
\end{itemize}

\begin{enumerate}
\item Underlying evolution follows unitarity (Information Volume Conservation);
...
\textbf{Gravity is the geometric projection of information conservation.}
