\section{3.1 The Discrete Origin of Light Cones: Deriving Maximum Signal Velocity $c$ from Lattice Hopping}
...
\subsection{3.1.1 Definition of Influence: Commutators as Causal Detectors}
...
\begin{quote}
\textbf{Definition (Causal Connection)}:
\begin{quote}
...
\textbf{Proof (Mathematical Induction)}:

\begin{enumerate}
\item \textbf{Heisenberg Evolution}: $\hat{O}_x(t) = (\hat{U}^\dagger)^t \hat{O}_x(0) \hat{U}^t$. This is equivalent to operator evolution backward in time.
\end{itemize}

\begin{enumerate}
\item \textbf{Base Case ($t=0$)}: If $x \neq y$, according to the tensor product structure of local Hilbert spaces, operators on different lattice points naturally commute. $[\hat{O}_x(0), \hat{O}_y(0)] = 0$. Theorem holds.

\begin{enumerate}
\item \textbf{Single-Step Diffusion ($t=1$)}:
\end{itemize}
...
\subsection{3.1.3 Ontological Definition of Light Speed $c$}
...
To connect with physical reality, we need to introduce \textbf{units}.

\begin{itemize}
\item Let the physical spacing between lattice points be Planck length $l_P$.
\end{enumerate}

\begin{itemize}
\item Let the physical duration of one logical update be Planck time $t_P$.
...
From this perspective, light speed $c$ is no longer a mysterious constant; it is the \textbf{aspect ratio of spacetime pixels}.

\begin{itemize}
\item \textbf{If $c$ were infinite}: It would mean $t_P \to 0$ or $l_P \to \infty$, corresponding to fully connected graphs or instantaneous computation, violating our discrete axiom.
\end{enumerate}

\begin{itemize}
\item \textbf{If $c$ were variable}: It would mean the lattice structure is non-uniform (non-translationally invariant). Under our uniform QCA assumption, $c$ must be a universal constant.

\subsection{3.1.4 Geometrization of Causality}

Theorem 3.1 not only defines speed, but also defines \textbf{geometric structure}.

On graph $\Lambda \times \mathbb{Z}$ (spacetime lattice), all point pairs $(y, 0)$ satisfying $d(x, y) \le r \cdot t$ constitute the \textbf{past light cone} of point $(x, t)$. Only events within this light cone can possibly be "causes" of $(x, t)$.

Conversely, all point pairs $(z, t)$ satisfying $d(x, z) \le r \cdot t$ constitute the \textbf{future light cone} of $(x, 0)$. Only events within this light cone can be "influenced" by $(x, 0)$.

In regions outside the light cone (spacelike separation), $[\hat{O}_x, \hat{O}_z] \equiv 0$. This means:

\textbf{For spacelike separated events, there is no objective temporal order.} Because there is no causal connection between them, which comes first or second will not lead to logical contradictions. This is precisely the microscopic origin of "relativity of simultaneity" in special relativity.

\subsection{3.1.5 Summary}
...
In this universe, \textbf{photons} are not special particles; they are \textbf{information wave packets that exactly reach the lattice propagation bandwidth limit}. They are the exposed skeleton of causal chains.
