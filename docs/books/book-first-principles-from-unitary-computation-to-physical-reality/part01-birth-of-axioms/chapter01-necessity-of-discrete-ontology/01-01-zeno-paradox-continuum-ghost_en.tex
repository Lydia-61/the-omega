\section{1.1 Zeno's Paradox and the Ghost of Continuum}
...
\subsection{1.1.1 Zeno's Dichotomy Paradox}
...
\subsection{1.1.2 The Information Density Catastrophe of Continuum}
...
This hides an astonishing physical cost: \textbf{infinite information density}.
...
\subsection{1.1.3 Ultraviolet Divergence in Quantum Field Theory}
...
\subsection{1.1.4 The Solution of Discrete Ontology}

Discrete ontology provides a radical solution: \textbf{What if Zeno was right?}
...
This leads to our first axiomatic foundation: \textbf{Physical reality has no infinitesimals.}

\subsubsection{Corollary 1.1.1 (Natural Cutoff)}
...
\subsubsection{Corollary 1.1.2 (Hilbert Space Finiteness)}

For any finite volume $V$, the number of orthogonal basis states it contains $N = \dim(\mathcal{H}_V)$ is a finite integer.

\subsection{1.1.5 The Necessity of Discreteness}

Once we accept discreteness, all infinite divergences instantly disappear. Zeno's paradox is resolved—motion is not gliding on a continuum, but \textbf{state updates} between discrete states. Achilles only needs to complete finitely many updates to reach the destination.
