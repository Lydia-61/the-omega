\section{1.2 Black Hole Entropy and Bekenstein Bound: The Universe as a Finite-Capacity Hard Drive}
...
\subsection{1.2.1 Bekenstein's Insight: Does Information Have Volume?}
...
\subsection{1.2.2 Planck Pixels and Finite Capacity}
...
This is a startling conclusion: \textbf{Information is not a continuously distributed fluid, but discretely "paved" on the surface of spacetime.}

Furthermore, Bekenstein proposed the \textbf{Bekenstein Bound}: For any spherical spatial region containing energy $E$ and radius $R$, the maximum entropy (i.e., maximum information) $S_{max}$ it can contain is finite and satisfies:
...
This means: \textbf{For any finite volume of space in the universe, no matter how we compress matter or energy into it, the total number of quantum states $W = e^S$ it can contain is strictly a finite integer.}

\subsection{1.2.3 The End of Continuum}
...
This provides the strongest physical support for our \textbf{discrete ontology}:

\begin{enumerate}
\item \textbf{Space is not a container, but a storage medium}: The geometric area of space directly corresponds to storage capacity (hard drive size). Planck length $l_P$ is the smallest magnetic domain (Bit) of this cosmic hard drive.
\end{itemize}

\begin{enumerate}
\item \textbf{Holographic Principle}: Since the maximum information in a volume is determined by its surface area, this suggests that the "bulk" information of three-dimensional space can actually be losslessly encoded on a two-dimensional boundary. Like holograms, this dimensional reduction encoding is mathematically possible only in discrete systems (continuum cardinalities differ, preventing one-to-one mapping).

\subsection{1.2.4 Conclusion: The Universe as a Finite State Machine}
...
If state space is finite and evolution rules are unitary (information conservation), then the universe is essentially equivalent to a \textbf{finite state machine} or a \textbf{quantum cellular automaton (QCA)} running on a huge but finite lattice.
