\section{5.1 Why Do Photons Have No Mass? — Pure Translation Modes}
...
\subsection{5.1.1 Definition 5.1 (Translation-Invariant Mode)}

Consider a one-dimensional simplified model of QCA. Let quantum state be $|\psi(x, t)\rangle$.

If evolution operator $\hat{U}$ merely maps state from $x$ to $x+a$ (or $x-a$) without changing its internal phase or spin structure, then this excitation is called a \textbf{Pure Translation Mode}.

\[\hat{U} |\psi(x)\rangle = |\psi(x \pm a)\rangle\]

\subsection{5.1.2 Theorem 5.1 (Massless Theorem)}
...
According to microscopic derivation of Light Path Conservation (see Chapter 3, Section 3.2), $v_{int}$ corresponds to parts of Hamiltonian that \textbf{anti-commute} with momentum $\hat{P}$ (i.e., mass term $\hat{M}$).

If $\hat{H}$ only has $\hat{P}$ term, then $\hat{M} \equiv 0 \implies v_{int} \equiv 0$.

Q.E.D.

\subsection{5.1.3 Physical Picture}
...
This also explains why gauge bosons (under unbroken symmetry) are usually massless: they are \textbf{messengers} for transmitting information in networks, not \textbf{nodes} for processing information.

So, what has mass? It must be things that \textbf{"stop to think."} In the next section, we will see that matter particles are precisely products of this "thinking."
